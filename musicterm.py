...
def search(content,limit):
    ...
    for video in search.result()["result"]:
        ...
        if len(title) < 40:
            title += " "*(40-len(title))
        print(Fore.LIGHTRED_EX+str(count)+Fore.RESET+": ("+Fore.BLUE+title+Fore.RESET+")---("+Fore.GREEN+video["duration"]+Fore.RESET+")")
        return_dict[count-1] = { "title": content.replace(" ","_"),"url":video["link"],"time": video["duration"]}
    option = input(Fore.YELLOW+"Select a song"+Fore.RESET+"~$ ")
...
def play_song(title):
    os.system("ffplay -autoexit -loglevel quiet -volume 20 -nodisp "+title+".mp4 ")
...
def display_playlists():
    ...
    for playlist in pla_l:
        count += 1
        print(Fore.RED+str(count)+Fore.RESET+": ("+Fore.BLUE+playlist["name"]+Fore.RESET+")---("+Fore.GREEN+str(len(playlist["songs"]))+Fore.RESET+")")
...
def display_songs(playlist):
    ...
    for song in playlists[playlist]["songs"]:
        count += 1
        print(Fore.RED+str(count)+Fore.RESET+": ("+Fore.BLUE+song["title"]+Fore.RESET+")---("+Fore.GREEN+song["time"]+Fore.RESET+")")
...
banner = pyfiglet.figlet_format("musicterm", font = "slant"  )
...
while(not exit):
    try:
        ...
        if y_s[0] == "download":
            ...
            son_l.append(song)
            print(Fore.BLUE+song["title"]+Fore.RESET+" downloaded!")
        if y_s[0] == "mkplaylist":
            pname = input(Fore.YELLOW+"Playlist Name"+Fore.RESET+"~$ ")
